...
#the load coordinate function is have x, z, y instead
    ...
    for i in range (1):
        fileName = "buildingData" + str(2) + ".csv"
    ...
    readResult = read_csv("buildingData2.csv")
    houseState = readResult.values.tolist()
    #remove the last one since it's action
  
    
    print("house data length: ", len(houseState))
    ...
    for i in range(210):
        renderState(houseState[214 - i], Point(x=0, y=4, z=0), Point(x=6, y=6, z=6))
