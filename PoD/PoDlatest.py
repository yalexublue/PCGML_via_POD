...
def fastGenMixVersion(houseData, min, max, paddingSize = 0, acceptedBlocks = [], fileName = "buildingData.csv"):
...
def spawnNBT(fileName="plain_village_house.nbt"):
        base_nbt_path = '../dataset/nbts/'
        nbt_path = "{}{}".format(base_nbt_path, fileName)
        print(nbt_path)
...
def preProcess(fileName = "plain_village_house.nbt"):
        houseData = spawnNBT(fileName)
    
        accurateMin = Point(x=50, y=2, z=10)
        accurateMax = Point(x=56, y=7, z=16)
        ...
        moveNBT(accurateMin, accurateMax, moveToCoord)
...
#the load coordinate function is have x, z, y instead
    ...
    for i in ["plain_village_house.nbt", "glasshouse.nbt", "mudhouse.nbt"]:
        house = preProcess(i)
        houseDatas.append(house)
        clearOut(newMinMax[0], newMinMax[1], AIR)
    ...
    # this is the slower generation method compare to the one used below
    # for i in range (1):
    #     fileName = "buildingData" + str(i) + ".csv"
    #     genEpisodes(currBuilding, newMinMax[0], newMinMax[1], 1, fileName, testData)

    # This is the faster data generation method compare to the one above
    #TODO: remember to switch agent between two csv file fomat to record data or render test
    
    for i in range(1):
        fileName = "buildingData" + str(i) + ".csv"
        iterNumber = 0
        for j in range(1):
            print("start new iteration: ", iterNumber)
            #currBuilding = houseDatas[random.randint(0,2)]
            currBuilding = houseDatas[1]
            #fastGenEpisodes(currBuilding, newMinMax[0], newMinMax[1], 1, Point(x=6,y=6,z=6), acceptedBlocks, fileName)
            fastGenMixVersion(currBuilding, newMinMax[0], newMinMax[1], Point(x=6,y=6,z=6), acceptedBlocks, fileName)
    ...
    # These lines are used to render csv files back to the game
    # readResult = read_csv("buildingData0.csv")
    # houseState = readResult.values.tolist()
    # renderState(houseState[215], Point(x=0, y=10, z=0), Point(x=13, y=13, z=13))

    # This is to repeately render state from bottom to top to see how house been reversed 
    # back to the original state
    # for i in range(210):
    #     renderState(houseState[214 - i], Point(x=0, y=10, z=0), Point(x=13, y=13, z=13))
  
   
    # These lines are used to reverse agent action to show how action can form the original house
    ...
    # These are lines to test the validity of the csv file, if column number is correct.
